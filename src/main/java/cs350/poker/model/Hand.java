 ...
 *
 * <p>A hand can hold zero or more {@link Card} objects. Cards can be added,
 * removed, found, and sorted. The hand supports sorting by rank (which
 * accounts for Ace-high) and by face value (natural numeric order).</p>
 *
 * @author Poker Demo
 * @version 1.0
 */
public class Hand {
    ...
    public List<Card> cards; // dynamic collection of cards in the hand
    ...
    /**
     * Removes the first occurrence of a card with the same face value and suit
     * as the specified card from this hand.
     *
    ...
    public boolean removeCard(Card card) {
        ...
        for (int i = 0; i < cards.size(); i++) {
            Card c = cards.get(i);
            if (c.getFaceNum() == card.getFaceNum()
                    && c.getSuitNum() == card.getSuitNum()) {
                cards.remove(i);
    ...
    public Card removeCard(int index) {
        if (index < 0 || index >= cards.size()) { // bounds check
            throw new IndexOutOfBoundsException(
                "Index " + index + " out of range for hand of size " + cards.size());
        }
    ...
    public Card findCard(Card card) {
        if (card == null) return null; // nothing to find
        for (Card c : cards) {
            if (c.getFaceNum() == card.getFaceNum()
                    && c.getSuitNum() == card.getSuitNum()) {
     ...
     *
     * <p>Uses the natural ordering defined by {@link Card#compareTo(Card)}.</p>
     */
     ...
     *
     * <p>This is a numeric sort on the face value field, treating Ace as 1
     * (the lowest). For Ace-high sorting, use {@link #sortByRank()} instead.</p>
     */
    ...
    /**
     * Returns the number of cards in this hand whose face value matches
     * the specified value.
     *
     * <p>For example, {@code numberOf(12)} returns the count of Queens.</p>
     *
     ...
     *
     * <p>This is useful for iteration without risking modification.</p>
     *
    ...
    /**
     * Returns a human-readable string representation of this hand.
     *
     * <p>Each card is listed on a line with its index for easy reference.</p>
     *
